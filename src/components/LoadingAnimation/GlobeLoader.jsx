...
class ArcballControl {
    ...
    constructor(canvas, updateCallback) {
        ...
        this.simluationTimeout = undefined;
    ...
    triggerSimulation() {
        // const randomDelay = Math.random() *1000  + 500; // 1000ms to 3000ms
        this.simluationTimeout = setTimeout(() => { this.simulatePointerEvent() }, 800);
    }

    stopSimulation(){
        clearTimeout(this.simluationTimeout)
    }
    ...
    simulatePointerEvent() {
        ...
        // Random start position inside the canvas
        const startX = Math.random() * rect.width;
        const startY = Math.random() * rect.height;
    
        // Define a fixed movement distance
        const moveDistance = 200; // Adjust as needed
        ...
        // Simulate pointer down at start position
        vec2.set(this.pointerPos, startX, startY);
        vec2.copy(this.previousPointerPos, this.pointerPos);
        this.isPointerDown = true;
    
        // Start movement animation
        this.animatePointer(startX, startY, endX, endY);
    }
    


    animatePointer(startX, startY, endX, endY) {
        ...
        // Set duration based on distance (e.g., 1px = 2ms) + some randomness
        const baseSpeed = 1; // Adjust speed factor (lower = faster)
        const randomFactor = Math.random() * distance / 3; // Randomness (-50ms to +50ms)
        const duration = Math.max(100, distance * baseSpeed + randomFactor); // Ensure minimum duration

        const easeInOutQuad = (t) => (t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2);

        const animate = (currentTime) => {
            let elapsed = currentTime - startTime;
            let t = Math.min(elapsed / duration, 1); // Normalize to range [0,1]
            let easedT = easeInOutQuad(t); // Apply easing function

            // Interpolated position
            let x = startX + easedT * dx;
            let y = startY + easedT * dy;
            ...
            if (t < 1) {
                ...
                this.isPointerDown = false; // End simulation
                this.triggerSimulation();
            }
        };
 ...
 * @property {Item[]} items - An array of items to display.
 */

/**
 * A React component that displays a list of items.
 * 
